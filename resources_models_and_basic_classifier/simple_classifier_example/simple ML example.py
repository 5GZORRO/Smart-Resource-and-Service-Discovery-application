...
try:
     ...
     addColumn = "ALTER TABLE RESOURCES ADD COLUMN classh CHAR(2)"
...
for i in range(1000):
  ...
  x=pandasDF.iloc[[i], [8]]
  classh=str(x.squeeze())
  try:
     sqliteConnection = sqlite3.connect(database)
     cursor = sqliteConnection.cursor()
     #print("Successfully Connected to SQLite")
    
     sql_update_query = """Update RESOURCES set classh = ? where id = ?"""
     data = (classh, i)
...
#maybe keep Scaled features.....?????? to think
...
spark2 = SparkSession.builder.master("aso") \
                    .appName('xxx') \
                    .getOrCreate()
...
predictionsxx = model.transform(dfnew)   
#predictionsxx.show()
#predictionsxx.select("prediction").show()
belongs_to_cluster=predictionsxx.select("prediction").collect()
leksh=str(belongs_to_cluster)
import re
temp0=re.findall("\d+\.\d+", leksh) 
...
sqlite_insert_query = """SELECT * from RESOURCES where classh==?"""
